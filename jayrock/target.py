...
class Target:
    ...
    # def query_thermal_flux(self):
    #     """Query thermal flux of target during visbility as seen from JWST using Miriade.
    #
    #     See https://ssp.imcce.fr/webservices/miriade/howto/ephemcc/
    #
    #     Notes
    #     -----
    #     Populates the following attribute:
    #         - thermal_flux : list of float
    #             Thermal flux in mJy at requested dates and wavelength.
    #     """
    #     if not hasattr(self, "visibility"):
    #         raise AttributeError("Run query_visibility() before query_thermal_flux().")
    #
    ...
    #     date = [
    #         float(Time(d, format="iso").jd) for d in self.visibility.date_obs.to_list()
    #     ]
    #     files = {  # file to submit with query
    #         "epochs": ("epochs", "\n".join([f"{d:.6f}" for d in date])),
    #     }
    #
    #     beaming = 1.0  # beaming parameter, assume 1
    #     emissivity = 0.9  # emissivity, assume 0.9
    #     wave = 15.0  # wavelength in microns
    #     albedo = self.rock.albedo.value if self.rock.albedo else 0.1
    #
    #     params = {
    #         "-name": f"a:{self.rock.name}",
    #         "-mime": "json",
    #         "-output": f"--thermalflux({wave},{albedo},{beaming},{emissivity})",
    #         "-observer": OBSERVER,
    #     }
    #
    #     # Execute query
    ...
    #     try:
    #         self.thermal_flux = (
    #             np.array([data["Thermal Flux"][0]["flux"] for data in j["data"]]) * 1000
    #         )
    #     except KeyError:
    #         raise ValueError(f"Thermal flux query failed: {j['message']}")
    #
    #     self.visibility["thermal_flux"] = self.thermal_flux
    #
    #     # Convenience attributes
    #     self.thermal_flux_min = self.thermal_flux.min()
    #     self.thermal_flux_max = self.thermal_flux.max()

    # @property
    # def surface_temperature(self):
    #     """NEATM subsolar temperature with MacLennan+ 2021 correction"""
    #     a = self.rock.orbital_elements.semi_major_axis.value
    #     albedo = self.rock.albedo.value if self.rock.albedo else 0.1
    #
    #     G = 0.15
    #     A = albedo * (0.290 + 0.684 * G)
    #     epsilon = 0.95  # emissivity
    #     # eta = 0.91  # beaming parameter  # Mainzer+ 2011
    #     eta = 1  # Masiero+ 2011
    #
    #     # TODO: Make function argument, replace by distance to Sun at time of observation
    #     a = self.rock.orbital_elements.semi_major_axis.value
    #
    #     Fsun = 1367.567 / a**2  # W / m2
    #     sigma = 5.670373e-08  # W / (K4 m2)
    #     Tss = (((1.0 - A) * Fsun) / abs(eta) / epsilon / sigma) ** 0.25
    #
    #     Tc = (Tss / 0.777) ** (1 / 1.063)  # MacLennan+ 2021, section 2.4
    #     return Tc
